...
El registro abarca la documentación académica contenida en el directorio `TESIS/` y su correspondencia con el código fuente del sistema, en la versión consignada en el archivo `VERSION` del repositorio. Comprende además la verificación de correspondencia con el portal de documentación alojado en `docs/`, que reproduce el mismo corpus documental para su consulta en el navegador.
