...
class Configuracion(Base, BaseModel):
    ...
    editable: Mapped[int] = mapped_column(Integer, default=1, nullable=False)
    ...
    def valor_typed(self):
        ...
        try:
            if self.tipo_dato == "int":
                ...
                return str(self.valor).strip().lower() in (
                    "true",
                    "1",
                    "yes",
                    "on",
    ...
    def set_valor(self, valor: Any) -> None:
        ...
        if valor is None:
            ...
            if isinstance(valor, str):
                self.valor = (
                    "true"
                    if valor.strip().lower() in ("true", "1", "yes", "on", "si", "verdadero")
                    else "false"
                )
            else:
