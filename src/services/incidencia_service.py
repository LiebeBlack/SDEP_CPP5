...
class IncidenciaService:
    ...
    def rechazar_incidencia(
        self, incidencia_id: int, rechazado_por: str, comentarios: str | None = None
    ) -> bool:
    ...
    def obtener_soporte(self, incidencia_id: int) -> bytes | None:
    ...
    def calcular_dias_incidencias_periodo(
        ...
        for incidencia in incidencias:
            ...
            if (
                ...
                inc_ini = (
                    ...
                    if isinstance(incidencia.fecha_inicio, date)
                    else parse_date(str(incidencia.fecha_inicio))
                )
                inc_fin = (
                    incidencia.fecha_fin
                    if isinstance(incidencia.fecha_fin, date)
                    else parse_date(str(incidencia.fecha_fin))
                )
    ...
    def validar_datos_incidencia(self, datos: dict) -> list[str]:
        ...
        if "fecha_inicio" in datos and "fecha_fin" in datos:
            f_ini = (
                parse_date(datos["fecha_inicio"])
                if isinstance(datos["fecha_inicio"], str)
                else datos["fecha_inicio"]
            )
            f_fin = (
                parse_date(datos["fecha_fin"])
                if isinstance(datos["fecha_fin"], str)
                else datos["fecha_fin"]
            )
