...
def format_date(fecha: date | datetime | str | None, formato: str = "%d/%m/%Y") -> str:
    """
...
def parse_date(fecha_str: str | date | datetime | None, formato: str = "%d/%m/%Y") -> date | None:
    """
...
def parse_currency(monto_str: str) -> float | None:
    """
    ...
    try:
        ...
        cleaned = (
            monto_str.replace("$", "").replace("€", "").replace("£", "").replace(",", "").strip()
        )
...
def ensure_directory_exists(directory_path: str) -> bool:
    ...
    try:
        ...
        logger.error("Error de permisos creando directorio %s: %s", directory_path, e)
        return False
    except OSError as e:
        logger.error("Error del sistema creando directorio %s: %s", directory_path, e)
        return False
    except Exception as e:
        logger.error("Error inesperado creando directorio %s: %s", directory_path, e)
        return False
...
def leer_archivo_seguro(ruta: str, reintentos: int = 3) -> bytes | None:
    """
...
def quitar_ventana_al_frente(window) -> None:
    ...
    try:
        ...
        logger.warning(
            "%s: operación auxiliar falló (se continúa)", "quitar_ventana_al_frente", exc_info=True
        )
