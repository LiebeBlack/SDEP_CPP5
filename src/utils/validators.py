...
class EmpleadoValidator(Validator):
    ...
    def validate_datos_empleado(datos: dict[str, Any]) -> list[str]:
        ...
        try:
            ...
            if (
                "salario_base" in datos
                and datos["salario_base"] is not None
                and str(datos["salario_base"]).strip() != ""
            ):
                try:
            ...
            if "fecha_nacimiento" in datos and datos["fecha_nacimiento"]:
                fnac = (
                    ...
                    else parse_date(str(datos["fecha_nacimiento"]))
                )
            ...
            if "fecha_contratacion" in datos and datos["fecha_contratacion"]:
                fcont = (
                    ...
                    if isinstance(datos["fecha_contratacion"], date)
                    else parse_date(str(datos["fecha_contratacion"]))
                )
    ...
    def validate_datos_incidencia(datos: dict[str, Any]) -> list[str]:
        ...
        try:
            ...
            if "fecha_inicio" in datos and "fecha_fin" in datos:
                ini = (
                    datos["fecha_inicio"]
                    if isinstance(datos["fecha_inicio"], date)
                    else parse_date(str(datos["fecha_inicio"]))
                )
                fin = (
                    datos["fecha_fin"]
                    if isinstance(datos["fecha_fin"], date)
                    else parse_date(str(datos["fecha_fin"]))
                )
                ...
                if ini and fin:
                    ...
                    errores.append(
                        "Las fechas de inicio y fin deben tener un formato válido (DD/MM/YYYY o YYYY-MM-DD)"
                    )
            ...
            if "dias_solicitados" in datos and datos["dias_solicitados"] is not None:
                try:
                    ...
                    errores.append("Los días solicitados deben ser un número entero válido")
        ...
        try:
            ...
            if "periodo_inicio" in datos and "periodo_fin" in datos:
                ...
                if ini and fin and fin < ini:
                    errores.append(
                        "La fecha fin del periodo no puede ser anterior a la fecha inicio"
                    )
            ...
            if (
                "monto_bruto" in datos
                and "monto_neto" in datos
                and datos["monto_bruto"] is not None
                and datos["monto_neto"] is not None
            ):
                try:
                    ...
                    errores.append("Los montos del pago deben ser numéricos válidos")
